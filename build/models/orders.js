...
class ordersStore {
    ...
    async create(o) {
        try {
            ...
            console.log(`Unable To Create A New Order`);
            throw new Error(`${error}`);
        }
    }
    ...
    async show(id) {
        try {
            const conn = await database_1.Client.connect();
            const sql = 'SELECT id, status, user_id FROM products WHERE id=$1;';
    ...
    async addProduct(quantity, orderId, productId) {
        ...
        try {
            const conn = await database_1.Client.connect();
            const sql = 'INSERT INTO order_products (quantity, orders_id, products_id) VALUES ($1, $2, $3) RETURNING *;';
